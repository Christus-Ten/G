...
module.exports = {
        ...
        onStart: async function ({ message, event, args, getLang }) {
                if (event.type == "message_reply") {
                        if (event.messageReply.attachments && event.messageReply.attachments.length > 0) {
                                ...
                                for (const attachment of event.messageReply.attachments) {
                                        if (attachment.type == "share" && attachment.share && attachment.share.url && attachment.share.url.includes("facebook.com")) {
                                                try {
                                                        const uid = await findUid(attachment.share.url);
                                                        msg += `UID: ${uid}\n`;
                                                } catch (e) {}
                                        }
                                }
                ...
                // Handle Share Contact (vCard) if available in the event
                if (event.attachments && event.attachments.length > 0) {
                        let msg = "";
                        for (const attachment of event.attachments) {
                                if (attachment.type == "share" && attachment.share && attachment.share.url) {
                                         try {
                                                const uid = await findUid(attachment.share.url);
                                                msg += `UID: ${uid}\n`;
                                        } catch (e) {}
