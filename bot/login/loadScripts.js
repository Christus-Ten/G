...
module.exports = async function (api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, createLine) {
        ...
        const aliasesData = globalData && typeof globalData.get === 'function' ? await globalData.get('setalias', 'data', []) : [];
        if (aliasesData) {
        ...
        for (const folderModules of folders) {
                const makeColor = typeof createLine === 'function' ? (folderModules == "cmds" ?
                        createLine("LOAD COMMANDS") :
                        createLine("LOAD COMMANDS EVENT")) : (folderModules == "cmds" ? "LOAD COMMANDS" : "LOAD COMMANDS EVENT");
